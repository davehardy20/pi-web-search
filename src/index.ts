...
export default function webSearchExtension(
	pi: ExtensionAPI,
	searchRuntime: SearchRuntime = createTavilySearchRuntime(),
) {
	...
	pi.registerCommand("web-search-status", {
		...
		handler: async (_args, _ctx) => {
			...
			const readiness = searchRuntime.readiness();
			const hasApiKey = readiness.tavilyApiKeySet ?? readiness.ready;
			sendVisibleMessage(
				[
					`${metadata.name} v${metadata.version}`,
					`source: ${metadata.sourcePath}`,
					`tavily_api_key: ${hasApiKey ? "set" : "not set"}`,
					`search_ready: ${readiness.ready ? "ready" : "not ready"}`,
				].join("\n"),
				{
					...
					searchReady: readiness.ready,
					searchProvider: readiness.providerName,
				},
			);
		},
	});
	...
	pi.registerTool({
		...
		label: "Web Search",
		description:
			"Search the web using Tavily API. Returns relevant results with summaries and source URLs.",
		promptSnippet:
		...
		parameters: Type.Object({
			...
			searchDepth: Type.Optional(
				StringEnum(["basic", "advanced"] as const, {
					description:
						"basic = fast, cheaper | advanced = deeper analysis, slower",
					default: "basic",
				}),
			),
		...
		async execute(_toolCallId, params, signal) {
			...
			try {
				const data = await searchRuntime.search(request);
				...
				lines.push(`_(Search took ${data.responseTime?.toFixed(2) ?? "?"}s)_`);
